...
async function extract(fileBuffer, mimeType) {
  ...
  const header = { supplierName: null, documentNumber: null, date: null };
  const lineItems = [];

  for (const entity of doc.entities || []) {
    if (entity.type === 'supplier_name') header.supplierName = entity.mentionText;
    if (entity.type === 'purchase_order_id' || entity.type === 'invoice_id') header.documentNumber = entity.mentionText;
